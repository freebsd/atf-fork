...
# SYNOPSIS
#
#   AX_PROG_LUA[([MINIMUM-VERSION], [TOO-BIG-VERSION], [ACTION-IF-FOUND],
#       [ACTION-IF-NOT-FOUND])]
...
[
  ...
  AS_IF([test "x$ac_cv_header_lua_h" = 'xyes'],
    [ dnl Make a program to print LUA_VERSION defined in the header.
      ...
      dnl to do this reliably. For now, assume versions match when cross
      dnl compiling.

      AS_IF([test "x$cross_compiling" != 'xyes'],
        [ AC_CACHE_CHECK([for Lua header version],
            [ax_cv_lua_header_version],
            [ _ax_lua_saved_cxxflags=$CXXFLAGS
               _ax_lua_saved_cppflags=$CPPFLAGS
...
int main(int argc, char ** argv)
{
  ...
  exit(EXIT_SUCCESS);
         ...
         $SED -n "s|^Lua \(@<:@0-9@:>@\{1,\}\.@<:@0-9@:>@\{1,\}\).\{0,\}|\1|p"`
              ...
              CPPFLAGS=$_ax_lua_saved_cppflags
              CXXFLAGS=$_ax_lua_saved_cxxflags
            ])
...
[
  ...
  m4_define_default([_AX_LUA_LIB_LIST],
    [ /usr/lib \
      /usr/local/lib \
    ])
  ...
  [ dnl First search for extra libs.
    ...
    for _ax_lib_path in _AX_LUA_LIB_LIST; do
      test ! -d "$_ax_lib_path" && continue

      AC_MSG_CHECKING([for Lua libraries in])
      AC_MSG_RESULT([$_ax_lib_path])

      AS_UNSET([ac_cv_search_lua_load])

      _ax_lua_saved_libs=$LIBS
      LIBS="$LIBS $LUA_LIB -L$_ax_lib_path"
      AC_SEARCH_LIBS([lua_load],
        [ lua$LUA_VERSION \
      ...
      AS_IF([test "x$_ax_found_lua_libs" = "xyes"],
        [ LUA_LIB_PATH="-L$_ax_lib_path"
          break])
    done
    AS_IF([test "x$ac_cv_search_lua_load" != 'xno' &&
           test "x$ac_cv_search_lua_load" != 'xnone required'],
      [LUA_LIB="$LUA_LIB_PATH $ac_cv_search_lua_load $_ax_lua_extra_libs"])
  ])
